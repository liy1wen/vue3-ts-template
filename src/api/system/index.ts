...
// 获取数据列表
export const getDataList = (url: string, data: any) =>
  lRequest.post<IResponse>({ url: `/${url}/list`, data })
...
export const createData = (url: string, data: any) =>
  lRequest.post<IResponse>({ url: `/${url}`, data })

// 编辑数据
export const updateData = (url: string, data: any) =>
  lRequest.patch<IResponse>({ url: `/${url}`, data })
